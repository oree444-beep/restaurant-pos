...
export default function App() {
  ...
  const [tableRowsSetting, setTableRowsSetting] = useState(() => load('tableRowsSetting', 2));
  ...
  useEffect(() => save('tableRowsSetting', tableRowsSetting), [tableRowsSetting]);
  ...
  function completePayment({ print = false } = {}) {
    ...
    setWarning(''); setToast(`${selectedTable}번테이블 ${currentPayment}결제 완료했습니다`); setTimeout(() => setToast(''), 3000);
  ...
  function exportCSV() {
    ...
    downloadText(`매출통계_${yy}${mm}${dd}_${hh}${min}.csv`, ['날짜,시간,테이블,이용방식,결제,외상단체,금액', ...rows].join('\n'));
  }
  ...
  const tableRows = useMemo(() => {
    ...
    const rows = Math.min(Math.max(1, Number(tableRowsSetting || 1)), count);
    const arr = Array.from({ length: count }, (_, i) => i + 1);
    const result = [];
    const base = Math.ceil(count / rows);
    for (let i = 0; i < rows; i++) result.push(arr.slice(i * base, (i + 1) * base));
    return result.filter(r => r.length > 0);
  }, [tableCount, tableRowsSetting]);
  ...
  return (
    <div className="app">
      ...
      <section className="tables">{tableRows.map((row, idx) => <div className="table-row" style={{ gridTemplateColumns: `repeat(${row.length}, minmax(0, 1fr))` }} key={idx}>{row.map(n => { const hasOrder = (orders[n] || []).length > 0; return <button key={n} onClick={() => setSelectedTable(n)} className={`table-btn ${selectedTable === n ? 'selected' : hasOrder ? 'has-order' : ''}`}>{n}번</button>; })}</div>)}</section>
